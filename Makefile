...
## Trigger CI release job by pushing a GitHub Tag with version defined in the plugin.json file
...
ifneq ($(HAS_SERVER),)
	...
	@cd server
	@find ./server -type f -name "*.go" -not -path "./vendor/*" -exec goimports -w {} \;
...
ifneq ($(HAS_SERVER),)
	mkdir -p server/dist;
	cd server && env GOOS=linux GOARCH=$(GOARCH) $(GO) build $(GO_BUILD_FLAGS) -o dist/plugin-linux-amd64;
	cd server && env GOOS=darwin GOARCH=$(GOARCH) $(GO) build $(GO_BUILD_FLAGS) -o dist/plugin-darwin-amd64;
	cd server && env GOOS=windows GOARCH=$(GOARCH) $(GO) build $(GO_BUILD_FLAGS) -o dist/plugin-windows-amd64.exe;
endif
